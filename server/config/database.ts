...
const createClipsTable = (pool: Pool): Promise<boolean> => {
    ...
    [...LabelConfig.labelNames, ...LabelConfig.issueNames].map((key: string) => {
        sql = sql + `${key} INT,`;
    });
    sql = sql.slice(0, -1) + ')';
...
const generateClipsInfoFromVideos = async () => {
...
const fillNewClipsTable = (pool: Pool) => {
...
const checkIfTableExists = async (sql: string, pool: Pool) => {
    return new Promise<boolean>((resolve, reject) => {
        pool.query(sql, (err: MysqlError, rows) => {
            if (err) {
                appLogger.error(err);
                reject(err);
            }
...
export const createTablesIfNotExists = async (pool: Pool) => {
